...
const Result = ({ id, title, price, description, category, imageUrl, rating }: ProductIT) => {
  ...
  return (
    <div id="product" className="product result" onClick={()=>{document.getElementById('modal' + id)!.style.display="block"}}>
      ...
      <span style={{left: "230px"}} className="category">{toTitleCase(category)}</span>
        </div>
      </div>
    </div>
  )
